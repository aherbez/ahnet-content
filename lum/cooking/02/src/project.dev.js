require = function() {
  ...
  CookingPapaController: [ function(require, module, exports) {
    ...
    var CookingPapaController = cc.Class({
      ...
      _clearIcons: function _clearIcons() {
        this.ingredientsParent.removeAllChildren();
        this.stubbedItems.forEach(function(itemStub) {
          -1 != itemStub.itemID && itemStub.node.removeAllChildren();
        });
      },
      _setupItemIcons: function _setupItemIcons() {
        ...
        var shelfItemIDs = [ 3, 14, 13, 15, 20, 21 ];
        var itemData = null;
        var newItem = null;
        ...
        this.stubbedItems.forEach(function(itemStub) {
          -1 != itemStub.itemID && _this._addNewItem(itemStub.itemID, itemStub.node);
        });
      },
      ...
      clearSelection: function clearSelection() {
        null != this.selectedServing && this.selectedServing.setSelected(false);
        this.selectedTools.forEach(function(item) {
      ...
      submitAction: function submitAction() {
        ...
        var actionHash = action.sort().join("_");
        this.actionsTaken.push(actionHash);
      ...
      finishCooking: function finishCooking() {
        var _this3 = this;
        var recipieStepHashes = this.recipie.getHashedSteps();
        cc.log(recipieStepHashes);
        var score = 5 * recipieStepHashes.length;
        var index = -1;
        recipieStepHashes.forEach(function(stepHash, i) {
          index = _this3.actionsTaken.indexOf(stepHash);
          if (index === i) {
            cc.log(stepHash + " CORRECT at loc " + i);
            score += 100;
          } else if (-1 !== index) {
            cc.log(stepHash + " PRESENT at loc " + i);
            score += 10;
          } else if (-1 === index) {
            cc.log(stepHash + " NOT FOUND");
            score -= 5;
          }
        });
        this.showEndScreen(this.stepsTaken, this.recipie.numSteps, score);
      },
      showEndScreen: function showEndScreen(stepsTaken, recipieSteps, score) {
        this.endScreen.setResultText(stepsTaken, recipieSteps, score);
      ...
      itemSelectedCallback: function itemSelectedCallback(itemController) {
        ...
        cc.log("clicked on " + itemController.itemData.name);
  ...
  EndScreenController: [ function(require, module, exports) {
    ...
    var EndScreenController = cc.Class({
      ...
      setResultText: function setResultText(stepsTaken, recipieSteps, score) {
        this.resultText.string = stepsTaken > recipieSteps ? stepsTaken - recipieSteps + " extra steps taken." : stepsTaken < recipieSteps ? recipieSteps - stepsTaken + " steps missed." : "Perfect!!!";
        this.scoreText.string = score + " points";
      }
    });
    cc._RF.pop();
  }, {} ],
  ...
  ItemController: [ function(require, module, exports) {
    ...
    var ItemController = cc.Class(_defineProperty({
      ...
      itemSelected: function itemSelected() {
        cc.log("clicked on " + this.itemData.name);
  ...
  ItemStub: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "b103233BmFDqJ19OpJ+8w7y", "ItemStub");
    "use strict";
    var ItemController = require("ItemController");
    var ItemStub = cc.Class({
      extends: cc.Component,
      properties: {
        itemID: {
          default: -1,
    ...
    var RecipieStep = cc.Class({
      ...
      applyAction: function applyAction(currentAction) {
        ...
        this.actions = this.actions.filter(function(action) {
          self._makeArrayHash(action) === currHash && (found = true);
        });
        cc.log("found " + found);
        return found;
      },
      checkForCompletion: function checkForCompletion() {
        ...
        var self = this;
        this.actions = this.actions.filter(function(action) {
          _this2.actionsTaken.hasOwnProperty(self._makeArrayHash(action)) || (complete = false);
        });
        return complete;
      }
    });
  ...
  Recipie: [ function(require, module, exports) {
    ...
    var Recipie = cc.Class({
      ...
      getHashedSteps: function getHashedSteps() {
        var hashes = [];
        this.steps.forEach(function(step) {
          hashes = hashes.concat(step.getHashes());
        });
        return hashes;
      },
